...
from ERExtraction.mvgnas_manager import GNNModule
import gc       
...
class Trainer(object):
    ...
    def __init__(self, args,configs):
        ...
        if self.configs.dataset == ADE or self.configs.dataset == ADE1:
                self.args.num_of_model = len(ADE_RELATION_TYPES)
                configs['num_of_model'] = len(ADE_RELATION_TYPES)
        elif self.configs.dataset == BIORELEX  or self.configs.dataset == BIORELEX1:
                self.args.num_of_model = len(BIORELEX_RELATION_TYPES)
                configs['num_of_model'] = len(BIORELEX_RELATION_TYPES)
    ...
    def build_model(self):
        ...
        from search_space.search_space import IncrementSearchSpace
        search_space_cls = IncrementSearchSpace()     
        search_space = search_space_cls.get_search_space()      
        from searchAlgorithm.mvgnas_controller import SimpleNASController
        from ERExtraction.mvgnas_manager import GNNModule
        self.submodel_manager = GNNModule(self.args, self.configs)
        self.search_space = search_space
        
        
        action_list = search_space_cls.generate_action_list(num_of_model=self.args.num_of_model, cell=self.args.num_of_cell, multi_architectures=  self.configs['multi_architectures'])
    ...
    def train_shared(self, max_step=50, gnn_list=None):
        ...
        for gnn in gnn_list:
            ...
            try:
                _, val_score = self.submodel_manager.trainfun(gnn, format=self.args.format)
    ...
    def train_controller(self):
        ...
        total_loss = 0
        # print('self.args.controller_max_step',self.args.controller_max_step)
