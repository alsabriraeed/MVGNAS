...
def load_data(dataset_str):
    ...
    test_idx_reorder = parse_index_file(
        "/public/home/hpc204708010/GraphNASTextclass/graphnas/data/ind.{}.test.index".format(dataset_str))
...
#    dataset_str = '20g'
    ...
    objects = []
    for i in range(len(names)):
        with open("/public/home/hpc204708010/GraphNASTextclass/graphnas/data/ind.{}.{}".format(dataset_str, names[i]), 'rb') as f:
            if sys.version_info > (3, 0):
                objects.append(pkl.load(f, encoding='latin1'))
            else:
                objects.append(pkl.load(f))
...
#  features combine the training and test x
    all_x = allx
    all_x = sp.vstack((all_x, tx))
    # print('all_x: ', all_x.shape)
    all_y = ally
    all_y = sp.vstack((all_y, ty))
    features = sp.vstack((allx, tx)).tolil()
    ...
    train_idx_orig = parse_index_file(
        "/public/home/hpc204708010/GraphNASTextclass/graphnas/data/{}.train.index".format(dataset_str))
...
#    print(adj.shape)
#    why????????????????????????????????????????????????????????????????????????
#    make the matric adjacency symmetric 
    adj = adj + adj.T.multiply(adj.T > adj) - adj.multiply(adj.T > adj)

    return all_x,all_y, allx, ally, adj, features, y_train, y_val, y_test, train_mask, val_mask, test_mask, train_size, test_size
